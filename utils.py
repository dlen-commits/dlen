...
class Dist(object):
    ...
    def add(self, preds):
        ...
        print('%s_batch_dist:' % self.name, sum_x / count, np.sqrt((sum_x2 / count) - (sum_x / count) ** 2))
        self.sum_x += sum_x
        self.sum_x2 += sum_x2
        self.count += count
    ...
    def show(self):
        print('total auc of %s: ' % self.name,
                    self.sum_x / self.count, np.sqrt((self.sum_x2 / self.count) - (self.sum_x / self.count) ** 2))
